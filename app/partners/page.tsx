...
const page = () => {
  return (
    <main>
      <h1 className='my-16 mb-24 text-center text-8xl font-bold'>
        Our <span className='text-red-500'>Partners</span>
      </h1>

      <div className='flex items-center justify-center pb-8'>
        <div className='flex w-[75%] flex-wrap justify-evenly gap-16 gap-y-20'>
          {Partners.map((partner, index) => (
            <a
              key={index}
              href={partner.link}
              target='_blank'
              className='flex items-center justify-center transition-all duration-150 ease-in-out hover:scale-[1.05]'
            >
              <Image
                src={partner.logo}
